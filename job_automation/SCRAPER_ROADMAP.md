...
**Phase 0 — groundwork (small, no behaviour change)**
- Extract a shared `HttpClient` (httpx + tenacity + hishel + per-host rate limit + robots check) and
  route `BaseScraper.get()` through it.
- Add `SEARCH_BACKEND` config and a `companies.yaml` for target slugs.

**Phase 1 — ATS engine (biggest win)**
- Implement `scrapers/ats_scraper.py` covering Greenhouse + Lever + Ashby + Workable.
- Seed `companies.yaml` with your real targets; map fields to `Job`; add offline fixture tests.
